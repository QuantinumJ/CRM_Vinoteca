...
export class ClienteService {
  ...
  protected convertDateFromClient(cliente: ICliente): ICliente {
    return Object.assign({}, cliente, {
      fechaAlta: cliente.fechaAlta?.isValid() ? cliente.fechaAlta.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((cliente: ICliente) => {
        cliente.fechaAlta = cliente.fechaAlta ? dayjs(cliente.fechaAlta) : undefined;
      });
    }
    return res;
  }
