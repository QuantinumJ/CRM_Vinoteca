...
export class EmpleadoService {
  ...
  protected convertDateFromClient(empleado: IEmpleado): IEmpleado {
    return Object.assign({}, empleado, {
      fechaAlta: empleado.fechaAlta?.isValid() ? empleado.fechaAlta.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((empleado: IEmpleado) => {
        empleado.fechaAlta = empleado.fechaAlta ? dayjs(empleado.fechaAlta) : undefined;
      });
    }
    return res;
  }
