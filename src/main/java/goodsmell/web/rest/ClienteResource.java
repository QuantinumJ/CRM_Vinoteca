...
import goodsmell.domain.Cliente;
import goodsmell.repository.ClienteRepository;
...
/**
 * REST controller for managing {@link goodsmell.domain.Cliente}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Cliente> partialUpdateCliente(
        ...
        Optional<Cliente> result = clienteRepository
            ...
            .map(existingCliente -> {
                ...
                if (cliente.getFidelizado() != null) {
                    existingCliente.setFidelizado(cliente.getFidelizado());
                }
