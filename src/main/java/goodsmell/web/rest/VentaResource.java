...
/**
 * REST controller for managing {@link goodsmell.domain.Venta}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Venta> result = ventaRepository
            ...
            .map(existingVenta -> {
                if (venta.getFechaHora() != null) {
                    existingVenta.setFechaHora(venta.getFechaHora());
                }
                if (venta.getCantidad() != null) {
                    existingVenta.setCantidad(venta.getCantidad());
                }
                if (venta.getTotalNeto() != null) {
                    existingVenta.setTotalNeto(venta.getTotalNeto());
                }
                if (venta.getTotalPagar() != null) {
                    existingVenta.setTotalPagar(venta.getTotalPagar());
                }
                if (venta.getTipoDePago() != null) {
                    existingVenta.setTipoDePago(venta.getTipoDePago());
                }
